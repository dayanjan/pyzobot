...
with st.sidebar:
    ...
    st.markdown("### Collection Filter (Optional)")
    ...
    if zotero_api_key and library_id and library_type:
        try:
            with st.spinner("Fetching PDFs..."):
                # Pass the collection_id to process_zotero_library
                temp_dir = process_zotero_library(
                    library_id, 
                    zotero_api_key, 
                    library_type,
                    collection_id if collection_id else None  # Only pass if not empty
                )
                
                if temp_dir:  # Check if directory was created successfully
                    ...
                    if pdf_files:
                        ...
                        st.write("No PDF files found in the specified collection.")
